...
| P4.3 — Separate platform delivery and state ownership | Tasks 1-3 done — all four Terraform roots (including `bootstrap`) transferred to [vinylhound-platform](https://github.com/jessig1/vinylhound-platform); a real production activation from that repository now passes provisioning, migration, rollout, and CloudFront smoke tests, with the final IAM/workflow single-writer cutover still pending. Task 4 remains open only for scaling and the authenticated pipeline smoke test | [p4.3-platform-delivery.md](roadmap/p4.3-platform-delivery.md)               |
