...
# Signs/verifies the service-identity tokens between apps/web and
# apps/discovery (ADR-0025's authenticated internal API, P4.1 Task 2/5).
# Generated by Terraform rather than supplied like the external Clerk/OpenAI
# secrets above: it is internal-only, never seen outside this environment,
# and both sides read the identical value from this one secret.
